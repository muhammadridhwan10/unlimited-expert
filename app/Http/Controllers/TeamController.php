...
class TeamController extends Controller
{
    ...
    public function index()
    {
        ...
        if(\Auth::user()->can('manage team'))
        {
            ...
            {
                $teams = User::where('type', '=', 'staff_client')->where('client_id', '=', $user->id)->get();
            }
            else
            {
                $teams = User::where('created_by', '=', $user->creatorId())->where('type', '!=', 'client')->get();
            }
    ...
    public function create()
    {
        ...
        {
            return view('team.create', compact('roles', 'customFields'));
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('create team'))
        {
            ...
            return redirect()->route('team.index')->with('success', __('User successfully created.') . ((!empty($resp) && $resp['is_success'] == false && !empty($resp['error'])) ? '<br> <span class="text-danger">' . $resp['error'] . '</span>' : ''));
    ...
    public function edit($id)
    {
        ...
        if(\Auth::user()->can('edit team'))
        {
            ...
            return view('team.edit', compact('user', 'roles', 'customFields'));
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
    ...
    //     return view('team.reset', compact('user'));
