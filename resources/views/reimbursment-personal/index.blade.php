...
@section('page-title')
    {{__('Manage Reimbursement Personal')}}
@endsection

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{route('dashboard')}}">{{__('Dashboard')}}</a></li>
    <li class="breadcrumb-item">{{__('Manage Reimbursement Personal')}}</li>
@endsection

@section('action-btn')
    <div class="float-end">
        <a href="#" data-size="lg" data-url="{{ route('reimbursment-personal.create') }}" data-ajax-popup="true" 
           data-bs-toggle="tooltip" title="{{__('Create')}}" data-title="{{__('Create Reimbursement Personal')}}" 
           class="btn btn-primary">
            <i class="ti ti-plus"></i> {{__('Add Reimbursement')}}
        </a>
    </div>
@endsection
...
@section('content')
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-lg-6 col-md-12 d-flex align-items-end">
                                <div class="btn-group w-100" role="group">
                                    ...
                                    <a href="{{ route('reimbursment-personal.index') }}" class="btn btn-secondary">
    ...
    <!-- Reimbursement Personal Records -->
    <div class="row">
        <div class="col-12">
            <div class="clean-card fade-in">
                <div class="card-header-clean">
                    <h6><i class="ti ti-receipt me-2"></i>{{__('Reimbursement Personal Records')}}</h6>
                </div>
                <div class="card-body p-0">
                    ...
                    @else
                        <div class="empty-state">
                            ...
                            <h5>{{__('No Reimbursement Records Found')}}</h5>
                            <p>{{__('There are no reimbursement personal records to display.')}}</p>
                        </div>
                    @endif
                </div>
            </div>
                    ...
                    <div class="card-body p-0">
                                    ...
                                    <tbody>
                                        @foreach ($approval as $approvals)
                                            <tr>
                                                ...
                                                <td data-label="Action">
                                                    <a href="#" data-url="{{ URL::to('reimbursment-personal/'.$approvals->id.'/action') }}" data-size="lg" data-ajax-popup="true" data-title="{{__('Reimbursment Action')}}" class="mx-3 btn btn-sm btn-action  align-items-center" data-bs-toggle="tooltip" title="{{__('Reimbursment Action')}}" data-original-title="{{__('Reimbursment Action')}}">
                        ...
                        @else
                            <div class="empty-state">
                                ...
                                <p>{{__('All reimbursement requests have been processed.')}}</p>
                            </div>
                        @endif
                    </div>
                </div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#approve-selected').click(function () {
                ...
                if (confirm('{{__("Are you sure you want to approve selected reimbursement requests?")}}')) {
                    var url = "{{ route('approve-multiple') }}";
                    var data = {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            alert('{{__("Reimbursement requests have been approved successfully.")}}');
        ...
        $(document).on('click', '.view-images', function () {
            var p_url = "{{route('reimbursment-personal.image.view')}}";
            var data = {
